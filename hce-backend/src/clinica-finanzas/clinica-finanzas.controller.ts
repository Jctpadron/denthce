...
export class ClinicaFinanzasController {
  ...
  async getPresupuestos(
    ...
    return this.finanzasService.getPresupuestos(req.user.tenantId, {
      patientId,
      estado,
    });
  }
  ...
  async createPresupuesto(
    ...
    return this.finanzasService.createPresupuesto(
      req.user.tenantId,
      dto,
      req.user.preferred_username || req.user.sub,
    );
  }
  ...
  async updatePresupuesto(
    @Param('id') id: string,
    @Body() dto: Partial<CreatePresupuestoDto>,
    @Request() req: any,
  ) {
    return this.finanzasService.updatePresupuesto(
      req.user.tenantId,
      id,
      dto,
      req.user.preferred_username || req.user.sub,
  ...
  async presentarPresupuesto(@Param('id') id: string, @Request() req: any) {
    return this.finanzasService.transicionarEstado(
      req.user.tenantId,
      id,
      'presentado',
    );
  ...
  async getPagos(
    ...
    return this.finanzasService.getPagos(req.user.tenantId, {
      patientId,
      presupuestoId,
    });
  }
  ...
  async registrarPago(@Body() dto: RegistrarPagoDto, @Request() req: any) {
    return this.finanzasService.registrarPago(
      req.user.tenantId,
      dto,
      req.user.preferred_username || req.user.sub,
    );
  ...
  async anularPago(
    ...
    return this.finanzasService.anularPago(
      req.user.tenantId,
      id,
      dto,
      req.user.preferred_username || req.user.sub,
  ...
  async getGastos(
    ...
    return this.finanzasService.getGastos(req.user.tenantId, {
      categoria,
      desde: desdeDate,
      hasta: hastaDate,
    });
  ...
  async registrarGasto(@Body() dto: CreateGastoDto, @Request() req: any) {
    return this.finanzasService.registrarGasto(
      req.user.tenantId,
      dto,
      req.user.preferred_username || req.user.sub,
    );
  ...
  async getCuentaCorriente(
    ...
    return this.finanzasService.getCuentaCorriente(
      req.user.tenantId,
      patientId,
    );
  }
  ...
  async getReporte(
    ...
    return this.finanzasService.getReporte(
      req.user.tenantId,
      new Date(desde),
      new Date(hasta),
    );
