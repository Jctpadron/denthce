...
export class OdontologyPdfService {
  ...
  private renderPage1(doc: typeof PDFDocument, patient: PatientEntity, resources: any[]) {
    ...
    const lesiones = comp('lesiones')?.valueBoolean ? 'SÍ' : 'NO';
    const lesionZona = comp('lesionZona')?.valueString || '—';
    const lesionTipo = comp('lesionTipo')?.valueString || '—';
    const diagnostico = comp('diagnostico')?.valueString || 'Sin diagnóstico registrado.';
    const plan = comp('plan')?.valueString || 'Sin plan registrado.';
    ...
    doc.font('Helvetica-Bold').text('Lesión Tejidos Blandos:', 360, y).font('Helvetica').text(lesiones, 470, y);

    if (comp('lesiones')?.valueBoolean) {
      y += 14;
      doc.font('Helvetica-Bold').text('Detalle lesión:', 45, y)
        .font('Helvetica').text(`Zona: ${lesionZona} | Tipo: ${lesionTipo}`, 120, y);
    }

    y += 18;
    doc.font('Helvetica-Bold').text('Diagnóstico Presuntivo:', 45, y);
    y += 11;
    ...
    y += 11;
    doc.font('Helvetica').fontSize(8).text(plan, 45, y, { width: 505, align: 'justify' });
  }
