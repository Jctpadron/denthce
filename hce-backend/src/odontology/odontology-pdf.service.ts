...
export class OdontologyPdfService {
  ...
  private renderPage1(
    ...
    // Fila 1 de datos
    doc
      .text('Nombre y Apellido:', 45, y)
      .font('Helvetica')
      .text(patientName, 140, y);
    doc
      .font('Helvetica-Bold')
      .text('DNI:', 320, y)
      .font('Helvetica')
      .text(patient.dni || 'N/D', 350, y);
    doc
      .font('Helvetica-Bold')
      .text('Fecha Nac.:', 440, y)
      .font('Helvetica')
      .text(birthStr, 500, y);
    ...
    doc
      .font('Helvetica-Bold')
      .text('Obra Social:', 45, y)
      .font('Helvetica')
      .text(coverage.obraSocial || 'Particular', 110, y);
    doc
      .font('Helvetica-Bold')
      .text('Nº Afiliado:', 180, y)
      .font('Helvetica')
      .text(coverage.subscriberId || 'N/D', 240, y);
    doc
      .font('Helvetica-Bold')
      .text('Nº Beneficio:', 340, y)
      .font('Helvetica')
      .text(coverage.beneficio || 'N/D', 400, y);

    y += 16;
    // Fila 3 de datos
    doc
      .font('Helvetica-Bold')
      .text('Médico Cabecera:', 45, y)
      .font('Helvetica')
      .text(coverage.medicoCabecera || 'N/D', 130, y);
    doc
      .font('Helvetica-Bold')
      .text('Cód. Prestador:', 320, y)
      .font('Helvetica')
      .text(coverage.prestador || 'N/D', 390, y);
    doc
      .font('Helvetica-Bold')
      .text('Titular:', 470, y)
      .font('Helvetica')
      .text(
        coverage.titular === false
          ? `No (${coverage.parentesco || 'familiar'})`
          : 'Sí',
        510,
        y,
      );
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(8)
      .text('Tratamiento Existente / Hallazgo (Rojo)', 75, y + 11);
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(8)
      .text('Tratamiento Requerido / Planificado (Azul)', 275, y + 11);
    ...
    const obs =
      resources.find(
        (r) =>
          ...
          r.code?.coding?.[0]?.code === 'oral-status',
      ) || {};
    const comp = (key: string) =>
      (obs.component || []).find((c: any) => c.code?.text === key);

    const placa = comp('placa')?.valueBoolean ? 'SÍ' : 'NO';
    const periodontal = comp('periodontal')?.valueBoolean ? 'SÍ' : 'NO';
    const diagnostico =
      comp('diagnostico')?.valueString || 'Sin diagnóstico registrado.';
    ...
    doc
      .font('Helvetica-Bold')
      .text('Enf. Periodontal:', 210, y)
      .font('Helvetica')
      .text(periodontal, 290, y);
    ...
    if (hallazgos.length > 0) {
      doc
        .font('Helvetica-Bold')
        .fontSize(8)
        .text('Examen de Tejidos Blandos:', 45, y);
      y += 11;
      hallazgos.forEach((h) => {
        doc
          .font('Helvetica')
          .fontSize(7.5)
          .text(`• ${h}`, 50, y, { width: 500 });
      ...
      doc
        .font('Helvetica')
        .fontSize(8)
        .text('Tejidos blandos: Sin hallazgos significativos.', 45, y);
      y += 12;
    }
    ...
    doc
      .font('Helvetica-Bold')
      .fontSize(8.5)
      .text('Diagnóstico Presuntivo:', 45, y);
    y += 11;
    doc
      .font('Helvetica')
      .fontSize(8)
      .text(diagnostico, 45, y, { width: 505, align: 'justify' });
    ...
    if (planItemsRaw) {
      try {
        planItems = JSON.parse(planItemsRaw);
      } catch {
        planItems = [];
      }
    }
    ...
    if (planItems.length === 0) {
      doc
        .font('Helvetica')
        .fontSize(8)
        .text('Sin procedimientos planificados.', 45, y, { width: 505 });
      y += 14;
    } else {
      // Table header
      ...
      planItems.forEach((item: any) => {
        const rowH = Math.max(
          16,
          doc.heightOfString(item.descripcion || '', { width: 200 }) + 6,
        );
        doc.fillColor('#0f172a').font('Helvetica').fontSize(7.5);
        doc.text(item.descripcion || '—', 50, y + 3, { width: 200 });
        doc
          .fillColor('#64748b')
          .text(item.prioridad || '—', 280, y + 3, { width: 80 });
        doc
          .fillColor('#0f172a')
          .text(item.estado || '—', 360, y + 3, { width: 80 });
    ...
    if (observaciones) {
      ...
      doc
        .font('Helvetica')
        .fontSize(8)
        .text(observaciones, 45, y, { width: 505, align: 'justify' });
    }
  }
  ...
  private renderPage2(
    ...
    const qr =
      resources.find((r) => r.resourceType === 'QuestionnaireResponse') || {};
    ...
    questions.forEach((q, idx) => {
      ...
      doc
        .font('Helvetica')
        .fillColor(val === true ? '#ef4444' : '#0f172a')
        .text(textVal + (detail ? ` (${detail})` : ''), qx + 130, qy, {
          width: 45,
        });
    ...
    const cigarrillos =
      fuma === true
        ? qrItems.find((i: any) => i.linkId === 'fuma')?.answer?.[1]
            ?.valueInteger || ''
        : '';
    ...
    doc
      .font('Helvetica-Bold')
      .text('Cepillados diarios:', 200, y)
      .font('Helvetica')
      .text(String(cepillados), 295, y);
    doc
      .font('Helvetica-Bold')
      .text('Momentos azúcar:', 360, y)
      .font('Helvetica')
      .text(String(azucar), 445, y);

    y += 15;
    doc
      .font('Helvetica-Bold')
      .text('Motivo de consulta:', 45, y)
      .font('Helvetica')
      .text(motivo, 135, y);
    ...
    const firmadoEl = consent.dateTime
      ? new Date(consent.dateTime).toLocaleString('es-AR')
      : 'Sin firma';
    ...
    const sigPaciente =
      consent.firmaPaciente ||
      (qr.extension || []).find(
        (e: any) =>
          e.url ===
          'http://denthce.local/fhir/StructureDefinition/patient-signature',
      )?.valueString;
    this.drawSignature(
      doc,
      sigPaciente,
  ...
  private renderPage3(
    ...
    const evolutions = resources
      .filter(
        (r) =>
          ...
          r.code?.coding?.[0]?.system === EVOLUTION_SYSTEM,
      )
      .sort((a, b) =>
        (a.performedDateTime || '').localeCompare(b.performedDateTime || ''),
      );
    ...
    if (evolutions.length === 0) {
      doc.text(
        'No hay entradas de evolución clínica registradas para este paciente.',
        55,
        y + 15,
      );
      y += 30;
      ...
      evolutions.forEach((ev) => {
        const dateStr = ev.performedDateTime
          ? new Date(ev.performedDateTime).toLocaleDateString('es-AR')
          : '—';
        const txt = ev.code?.text || '—';
        const conf = ev.conformidad
          ? 'SÍ (CONFORME)'
          : 'NO CONFORME / PENDIENTE';

        const rowHeight = Math.max(
          20,
          doc.heightOfString(txt, { width: 260 }) + 10,
        );
        ...
        if (y + rowHeight > 700) {
          ...
          this.drawHeader(
            doc,
            'ANEXO DE EVOLUCIÓN CLÍNICA Y CONFORMIDAD (CONT.)',
            3,
          );
          y = 85;
        ...
        doc
          .font('Helvetica-Bold')
          .fillColor(ev.conformidad ? '#10b981' : '#ef4444')
          .text(conf, 420, y + 6);
    ...
    doc
      ...
      .fontSize(7.5)
      .text(
        'Aclaración de conformidad: Las prestaciones descritas arriba fueron recibidas de conformidad en las fechas indicadas, autorizando su cobro correspondiente a la obra social.',
        52,
        y + 8,
        { width: 490 },
      );
  ...
  private drawSignature(
    ...
    if (signatureStr && signatureStr.includes('data:image')) {
      ...
      doc
        ...
        .font('Helvetica')
        .fontSize(7)
        .text('(Sin firma digitalizada)', x + 38, y + 20);
    }
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(8)
      .text(label, x, y + 52, { align: 'center', width: 150 });
  }

  /**
   * Dibuja el odontograma vectorial detallado (arcada superior e inferior)
   */
  private drawVisualOdontogram(
    doc: typeof PDFDocument,
    resources: any[],
    y: number,
  ): number {
  ...
  private drawTooth(
    ...
    const resThisTooth = toothResources.filter(
      (r) => r.bodySite.coding[0].code === toothCode,
    );

    // Deducir estado del diente completo
    const isAusente = resThisTooth.some(
      (r) => r.code?.coding?.[0]?.code === '272673000',
    ); // SNOMED ausente
    const extraccionResource = resThisTooth.find(
      (r) => r.code?.coding?.[0]?.code === '65546002',
    ); // SNOMED extraccion
    const coronaResource = resThisTooth.find(
      (r) => r.code?.coding?.[0]?.code === '172922005',
    ); // SNOMED corona
    const endoResource = resThisTooth.find(
      (r) =>
        r.code?.coding?.[0]?.code === '234961008' ||
        r.code?.coding?.[0]?.code === '42425007',
    );
    ...
    const getLayerColor = (res: any) =>
      getLayer(res) === 'planned' ? '#2962ff' : '#ef4444';
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(7.5)
      .text(toothCode, cx - 8, cy - 20, { align: 'center', width: 16 });
    ...
    const isCuadranteDerecho =
      toothCode.startsWith('1') || toothCode.startsWith('4');
    ...
    Object.keys(facePolys).forEach((polyKey) => {
      ...
      const resFace = resThisTooth.find(
        (r) => r.bodySite.coding[1]?.code === dbFaceCode,
      );
      ...
      if (resFace) {
        ...
        if (snomed === '23450005' || snomed === '80967001') {
          ...
          doc
            .moveTo(poly.pts[0][0] + 2, midY)
            .lineTo(poly.pts[1][0] - 2, midY)
            .stroke();
    ...
    if (extraccionResource) {
      ...
      doc
        .moveTo(cx - r - 2, cy - r - 2)
        .lineTo(cx + r + 2, cy + r + 2)
        .stroke();
      doc
        .moveTo(cx + r + 2, cy - r - 2)
        .lineTo(cx - r - 2, cy + r + 2)
