...
describe('ClinichatOrchestrationService (Fase 4B)', () => {
  ...
  beforeEach(async () => {
    ...
    service = mod.get<ClinichatOrchestrationService>(
      ClinichatOrchestrationService,
    );
    jest.clearAllMocks();
  ...
  it('enableWhatsapp: genera SA, firma HMAC correcta y POSTea el payload enable', async () => {
    ...
    tenantConfigRepo.findOne.mockResolvedValue({
      tenantId: 't1',
      hceWebhookSecret: 'wh-secret',
    });
    fetchMock.mockResolvedValue({
      ok: true,
      status: 200,
      text: async () => '{"synced":true}',
    });
    ...
    expect(payload.credentials.hce_keycloak_grant_type).toBe(
      'client_credentials',
    );
    // La firma del header debe coincidir con el HMAC del body crudo.
    const expected =
      'sha256=' +
      crypto.createHmac('sha256', 'test_secret_123').update(body).digest('hex');
  ...
  it('rechaza enable sin pairing code', async () => {
    await expect(service.enableWhatsapp('t1', '')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('propaga error si CliniChat responde 404 (pairing no encontrado)', async () => {
    ...
    await expect(service.enableWhatsapp('t1', 'BAD')).rejects.toThrow(
      HttpException,
    );
  });
