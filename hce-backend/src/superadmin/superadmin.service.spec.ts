...
describe('SuperAdminService (cross-tenant)', () => {
  ...
  const keycloakAdmin = {
    createUser: jest.fn(),
    createClinicServiceAccount: jest.fn(),
  };
  ...
  beforeEach(async () => {
    const mod: TestingModule = await Test.createTestingModule({
      providers: [
        SuperAdminService,
        {
          provide: getRepositoryToken(TenantConfigEntity),
          useValue: tenantConfigRepo,
        },
        {
          provide: getRepositoryToken(TenantModuleEntity),
          useValue: tenantModuleRepo,
        },
        {
          provide: getRepositoryToken(PlatformModuleEntity),
          useValue: platformModuleRepo,
        },
  ...
  describe('setModule', () => {
    it('no permite dar de baja un módulo base', async () => {
      tenantConfigRepo.findOne.mockResolvedValue({ tenantId: 't1' });
      platformModuleRepo.findOne.mockResolvedValue({
        key: 'hc-base',
        isBase: true,
      });
      await expect(service.setModule('t1', 'hc-base', false)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('NotFound si la clínica no existe', async () => {
      tenantConfigRepo.findOne.mockResolvedValue(null);
      await expect(service.setModule('tx', 'whatsapp', true)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('si la orquestación de WhatsApp falla, NO se anexa el entitlement', async () => {
      ...
      await expect(
        service.setModule('t1', 'whatsapp', true, null, 'BAD'),
      ).rejects.toThrow();
    ...
    it('dar de baja WhatsApp orquesta CliniChat (disable)', async () => {
      ...
      tenantModuleRepo.findOne.mockResolvedValue({
        tenantId: 't1',
        moduleKey: 'whatsapp',
        enabled: true,
      });
  ...
  describe('generateServiceAccount', () => {
    it('genera el service-account si la clínica existe', async () => {
      tenantConfigRepo.findOne.mockResolvedValue({ tenantId: 't1' });
      keycloakAdmin.createClinicServiceAccount.mockResolvedValue({
        clientId: 'clinichat-t1',
        clientSecret: 'sec',
        tenantId: 't1',
      });
      const res = await service.generateServiceAccount('t1');
      expect(res.clientId).toBe('clinichat-t1');
      expect(keycloakAdmin.createClinicServiceAccount).toHaveBeenCalledWith(
        't1',
      );
    });
    ...
    it('NotFound si la clínica no existe', async () => {
      tenantConfigRepo.findOne.mockResolvedValue(null);
      await expect(service.generateServiceAccount('tx')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
