...
- [ ] **GOV.5:** Resolver el desalineo de `.github/workflows/deploy.yml` (`application_name: hce-backend` + `environment_name: HceBackend-env`) contra el prod REAL (`Odontocloud-env`; app EB a confirmar en AWS con `aws elasticbeanstalk describe-environments`). El deploy real hoy es manual vía `aws/scripts/deploy-aws.ps1` (correcto); `deploy.yml` (CI) nunca se usó. Decidir: alinear `deploy.yml` a la infra real **o** retirarlo si el deploy se mantiene por PowerShell. Conocido desde handoff 2026-06-18. *(Prioridad: Baja)*
